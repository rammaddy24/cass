...
import { Container, Content, connect, Picker, Header, LinearGradient, Snackbar, Moment, base64, CalendarPicker,ImgToBase64, SignatureCapture, DocumentPicker } from '../../../../Asset/Libraries/NpmList';
...
class Add_Timesheet extends Component {
    ...
    async Qty_ListMethod(Route_Data) {
        ...
        const Quatitylist_Response = await this._fetch_QtyInfo(this.state.CassUserID, this.state.CassRoleID)
        ...
        this.setState({
            ...
            S1_Engineer_DataArray: Array.from(new Set(S1_Engineer_Arraylist)),
    ...
    Imagesupload_Server(Image_Path) {
        ImgToBase64.getBase64String(Image_Path)
            .then(base64String =>
                
                this.UploadPhoto_API(base64String)
            )
            .catch(err =>
                Snackbar.show({
                    title:"Please try again Later..!",
                    duration: Snackbar.LENGTH_SHORT,
                    backgroundColor: LG_BG_THEME.Snackbar_theme,
                }));
    }
        ...
        if (RouteName == "Open") {
            ...
            if (this.state.S3_Section_No == "" && this.state.S3_Distance == "" && this.state.S3_Blockage == "" && this.state.S3_Desilt == "" && this.state.S3_New_Track == "" && this.state.S3_DFESlipNumber == "" && this.state.S3_Comments == "") {
                ...
                if (RouteName == "More") {
                    ...
                    this.setState({
                        S3_Section_No: "", S3_Distance: "", S3_Blockage: "",
                        S3_Desilt: "", S3_New_Track: "", S3_DFESlipNumber: "", S3_Comments: "", S3_Infostatus: false, AddionalInfo_Modal: false
                    })
                }
                                                                    ...
                                                                    {item.additional_info == "1" ?
                                                                        <View style={{ flex: 1, justifyContent: 'center', position: "absolute", flexDirection: "row", top: - (width / 100 * 1) }}>
                                                                            ...
                                                                            <TouchableOpacity onPress={() => this.S2_InfoMethod(item, this.state.S2_Quatitylist_Response)} style={{ flex: 0.2, justifyContent: "flex-start", alignItems: "flex-start", }}>
                                                                                <Image source={require('../../../../Asset/Icons/Ball_Info.png')} style={{ width: width / 100 * 5, height: width / 100 * 5, tintColor: item.isClicked == true ?  LG_BG_THEME.WHITE_THEME : LG_BG_THEME.APPTHEME_1 , transform: [{ rotate: '0deg' }] }} />
                                                                            </TouchableOpacity>
